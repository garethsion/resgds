...
# Layout filename
layout_file ='fab_files/reflective_bragg.gds'
...
ocirc = [rs.rect((2*gcav)+wcav,gcav,x[cnt],yb_strt)] 
ocirc_rem = [rs.rect(rm_width,3*gcav,x[cnt]-(rm_width/2)+(wcav/2)+gcav,yb_strt)] 

for i in range(0,len(ocirc)):
	oc = gdspy.Polygon(ocirc[i],2)
	poly_cell.add(oc)

for i in range(0,len(ocirc_rem)):
	oc_r = gdspy.Polygon(ocirc_rem[i],3)
	poly_cell.add(oc_r)
...
feed += rt.feedbond(x4,y4,wcav,gcav,feedlength=400,bondlength=800,bondh=200,orientation='E')
...
feedr += rt.feedbond_remove(x4r,y4r+rm_width/2-wlow/2-glow,wcav,gcav,feedlength=400,bondlength=800,bondh=200,orientation='E')
