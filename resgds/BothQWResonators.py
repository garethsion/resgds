...
# Layout filename
layout_file ='Both_quarter_wave.gds'
...
def quarterwave(wlow,glow,rqw,xq0,yq0,lo=50,feedline_sep=30):

	xq1, yq1 = [coords(xq0,-glow),coords(yq0,-feedline_sep)]
	qw = [rs.rect(glow,wlow+2*glow,xq1, yq1)]

	xq2, yq2 = [coords(xq0),coords(yq0,-feedline_sep)]
	qw += rs.straight_trench(ls,wlow,glow, xq2, yq2, orientation='H')

	xq3, yq3 = [coords(xq2,ls),coords(yq2,-rqw)]
	qw += rs.halfarc_trench(rqw,wlow,glow,xq3,yq3,orient='E',npoints=40)

	xq4, yq4 = [coords(xq3),coords(yq3,-rqw-wlow-2*glow)]
	qw += rs.straight_trench(-ls,wlow,glow, xq4, yq4, orientation='H')

	xq5, yq5 = [coords(xq4,-ls),coords(yq4,-rqw)]
	qw += rs.halfarc_trench(rqw,wlow,glow,xq5,yq5,orient='W',npoints=40)

	xq6, yq6 = [coords(xq5),coords(yq5,-rqw-wlow-2*glow)]
	qw += rs.straight_trench(ls,wlow,glow, xq6, yq6, orientation='H')

	xq7, yq7 = [coords(xq6,ls),coords(yq6,-rqw)]
	qw += rs.halfarc_trench(rqw,wlow,glow,xq7,yq7,orient='E',npoints=40)

	xq8, yq8 = [coords(xq7),coords(yq7,-rqw-wlow-2*glow)]
	qw += rs.straight_trench(-ls,wlow,glow, xq8, yq8, orientation='H')

	xq9, yq9 = [coords(xq8,-ls),coords(yq8,-rqw)]
	qw += rs.halfarc_trench(rqw,wlow,glow,xq9,yq9,orient='W',npoints=40)

	xq10, yq10 = [coords(xq9),coords(yq9,-rqw-wlow-2*glow)]
	qw += rs.straight_trench(ls,wlow,glow, xq10, yq10, orientation='H')

	xq11, yq11 = [coords(xq10,ls),coords(yq10,-rqw)]
	qw += rs.halfarc_trench(rqw,wlow,glow,xq11,yq11,orient='E',npoints=40)

	xq12, yq12 = [coords(xq11),coords(yq11,-rqw-wlow-2*glow)]
	qw += rs.straight_trench(-ls,wlow,glow, xq12, yq12, orientation='H')

	xq13, yq13 = [coords(xq12,-ls),coords(yq12,-rqw)]
	qw += rs.quarterarc_trench(rqw,wlow,glow,xq13,yq13,orient='NW',npoints=40)

	xq14, yq14 = [coords(xq13,-rqw-2*glow-wlow),coords(yq13)]
	qw += rs.straight_trench(-lo,wlow,glow, xq14, yq14, orientation='V')

	return qw

def quarterwave_remove(wlow,glow,rqw,xq0,yq0,lo=50,feedline_sep=30):

	rm_width = wlow + 4*glow
	arcrad = rqw - rm_width/2 + glow + wlow/2

	xq2, yq2 = [coords(xq0,-2*glow),
		coords(yq0,-feedline_sep + glow + wlow/2 - rm_width/2)]
	qwr = [rs.rect(ls+2*glow,rm_width, xq2, yq2)]

	xq3, yq3 = [coords(xq2,ls+2*glow),coords(yq2,-arcrad)]
...
wlow, glow, lqw = [30.44,6.685,4051.32]  # Low Z section
whigh, ghigh, lqw = [2,20.905,4051.32] # High Z section
wtot = 2*gfeed+wfeed
...
#########################################################################
rqw = 30
larc = arclength(wlow,glow,rqw)
lo = 50
ls = ( lqw - (5*larc) - (larc/2) - lo ) / 6

xlow0, ylow0 = [coords(x5,(lfeed2/4)-(ls/2)),coords(y5,-wlow-2*glow)]
qw = quarterwave(wlow,glow,rqw,xlow0,ylow0,lo=50,feedline_sep=30)

xhigh0, yhigh0 = [coords(x5,(3*lfeed2/4)-(ls/2)),coords(y5,-wlow-2*glow)]
qw += quarterwave(whigh,ghigh,rqw,xhigh0,yhigh0,lo=50,feedline_sep=30)
...
qwr = quarterwave_remove(wlow,glow,rqw,xlow0,ylow0,lo=50,feedline_sep=30)
qwr += quarterwave_remove(whigh,ghigh,rqw,xhigh0,yhigh0,lo=50,feedline_sep=30)
