...
class ResTempFiles:
	...
	def feedbond(self,x0,y0,w,g,feedlength=300,bondlength=600,bondh=150,orientation='N'):
		...
		if(orientation == 'N'):
			...
			feed = [self.rs.rect(bondw,bondh, xbond, ybond)]
			feed += [self.rs.rect(bondh,feedlength,xbond,ybond+bondh)]
			feed += [self.rs.rect(bondh,feedlength,xbond+bondw-bondh,ybond+bondh)]

			xa = xbond
			ya = ybond+bondw-bondh
			
			xb = xa+feedlength
			# yb = ya+bondw-g/2+w-feedlength
			yb = ya+bondw-feedlength

			d1 = [(xa, ya), (xa+bondh, ya), (xb-g-w, yb), (xb-2*g-w, yb)]
			d2 = [(xa+bondw-bondh, ya), (xa+bondw, ya), (xb, yb), (xb-g, yb)]
		...
		if(orientation == 'E'):
			...
			feed = [self.rs.rect(bondh,bondw, xbond, ybond)]
			feed += [self.rs.rect(feedlength,bondh,xbond+bondh,ybond)]
			feed += [self.rs.rect(feedlength,bondh,xbond+bondh,ybond+bondw-bondh)]
		...
		if(orientation == 'W'):
			...
			feed = [self.rs.rect(bondh,bondw, xbond, ybond)]
			feed += [self.rs.rect(feedlength,bondh,xbond-2*bondh,ybond)]
			feed += [self.rs.rect(feedlength,bondh,xbond-2*bondh,ybond+bondw-bondh)]
	...
	def feedbond_remove(self,x0,y0,w,g,feedlength=300,bondlength=750,bondh=150,orientation='E'):
		...
		# fbondr = [self.rs.rect(bondwr,bondwr-2*bondhr, xbond, ybond)]
	...
	def quarterwave(wqw,gqw,rqw,xq0,yq0,lo=50,feedline_sep=30):
		...
		qw = [self.rs.rect(gqw,wqw+2*gqw,xq1, yq1)]

		xq2, yq2 = [self.coords(xq0),self.coords(yq0,-feedline_sep)]
		qw += self.rs.straight_trench(ls,wqw,gqw, xq2, yq2, orientation='H')

		xq3, yq3 = [self.coords(xq2,ls),self.coords(yq2,-rqw)]
		qw += self.rs.halfarc_trench(rqw,wqw,gqw,xq3,yq3,orient='E',npoints=40)

		xq4, yq4 = [self.coords(xq3),self.coords(yq3,-rqw-wqw-2*gqw)]
		qw += self.rs.straight_trench(-ls,wqw,gqw, xq4, yq4, orientation='H')

		xq5, yq5 = [self.coords(xq4,-ls),self.coords(yq4,-rqw)]
		qw += self.rs.halfarc_trench(rqw,wqw,gqw,xq5,yq5,orient='W',npoints=40)

		xq6, yq6 = [self.coords(xq5),self.coords(yq5,-rqw-wqw-2*gqw)]
		qw += self.rs.straight_trench(ls,wqw,gqw, xq6, yq6, orientation='H')

		xq7, yq7 = [self.coords(xq6,ls),self.coords(yq6,-rqw)]
		qw += self.rs.halfarc_trench(rqw,wqw,gqw,xq7,yq7,orient='E',npoints=40)

		xq8, yq8 = [self.coords(xq7),self.coords(yq7,-rqw-wqw-2*gqw)]
		qw += self.rs.straight_trench(-ls,wqw,gqw, xq8, yq8, orientation='H')

		xq9, yq9 = [self.coords(xq8,-ls),self.coords(yq8,-rqw)]
		qw += self.rs.halfarc_trench(rqw,wqw,gqw,xq9,yq9,orient='W',npoints=40)

		xq10, yq10 = [self.coords(xq9),self.coords(yq9,-rqw-wqw-2*gqw)]
		qw += self.rs.straight_trench(ls,wqw,gqw, xq10, yq10, orientation='H')

		xq11, yq11 = [self.coords(xq10,ls),self.coords(yq10,-rqw)]
		qw += self.rs.halfarc_trench(rqw,wqw,gqw,xq11,yq11,orient='E',npoints=40)

		xq12, yq12 = [self.coords(xq11),self.coords(yq11,-rqw-wqw-2*gqw)]
		qw += self.rs.straight_trench(-ls,wqw,gqw, xq12, yq12, orientation='H')

		xq13, yq13 = [self.coords(xq12,-ls),self.coords(yq12,-rqw)]
		qw += self.rs.quarterarc_trench(rqw,wqw,gqw,xq13,yq13,orient='NW',npoints=40)

		xq14, yq14 = [self.coords(xq13,-rqw-2*gqw-wqw),self.coords(yq13)]
		qw += self.rs.straight_trench(-lo,wqw,gqw, xq14, yq14, orientation='V')
	...
	def quarterwave_remove(self,wqw,gqw,rqw,xq0,yq0,lo=50,feedline_sep=30):
		...
		qwr = [self.rs.rect(ls+2*gqw,rm_width, xq2, yq2)]

		xq3, yq3 = [self.coords(xq2,ls+2*gqw),self.coords(yq2,-arcrad)]
		qwr += [self.rs.halfarc(arcrad,rm_width,xq3,yq3,orientation='E',npoints=40)]

		xq4, yq4 = [self.coords(xq3),self.coords(yq3,-arcrad-rm_width)]
		qwr += [self.rs.rect(-ls,rm_width, xq4, yq4)]

		xq5, yq5 = [self.coords(xq4,-ls),self.coords(yq4,-arcrad)]
		qwr += [self.rs.halfarc(arcrad,rm_width,xq5,yq5,orientation='W',npoints=40)]

		xq6, yq6 = [self.coords(xq5),self.coords(yq5,-arcrad-rm_width)]
		qwr += [self.rs.rect(ls,rm_width, xq6, yq6)]

		xq7, yq7 = [self.coords(xq6,ls),self.coords(yq6,-arcrad)]
		qwr += [self.rs.halfarc(arcrad,rm_width,xq7,yq7,orientation='E',npoints=40)]

		xq8, yq8 = [self.coords(xq7),self.coords(yq7,-arcrad-rm_width)]
		qwr += [self.rs.rect(-ls,rm_width, xq8, yq8)]

		xq9, yq9 = [self.coords(xq8,-ls),self.coords(yq8,-arcrad)]
		qwr += [self.rs.halfarc(arcrad,rm_width,xq9,yq9,orientation='W',npoints=40)]

		xq10, yq10 = [self.coords(xq9),self.coords(yq9,-arcrad-rm_width)]
		qwr += [self.rs.rect(ls,rm_width, xq10, yq10)]

		xq11, yq11 = [self.coords(xq10,ls),self.coords(yq10,-arcrad)]
		qwr += [self.rs.halfarc(arcrad,rm_width,xq11,yq11,orientation='E',npoints=40)]

		xq12, yq12 = [self.coords(xq11),self.coords(yq11,-arcrad-rm_width)]
		qwr += [self.rs.rect(-ls,rm_width, xq12, yq12)]

		xq13, yq13 = [self.coords(xq12,-ls),self.coords(yq12,-arcrad)]
		qwr += [self.rs.quarterarc(arcrad,rm_width,xq13,yq13,orientation='NW',npoints=40)]

		xq14, yq14 = [self.coords(xq13,-arcrad-rm_width),self.coords(yq13)]
		qwr += [self.rs.rect(rm_width,-lo, xq14, yq14)]
	...
	def halfwaveresonator(self,x1,y1,w,g,r,rfeed,arc,lcap,l1,l2,l3):
		...
		resonator = self.rs.straight_trench(l1, w, g, x1, y1, orientation='H')

		x3, y3 = [ self.coords(x2,l1-lcap), self.coords(y1,rfeed+wtot) ]
		resonator += self.rs.quarterarc_trench(rfeed, w, g, x3, y3, orient='SE')

		x4, y4 = [ self.coords(x3,rfeed), self.coords(y3) ]
		resonator += self.rs.straight_trench(l2+arc/2, w, g, x4, y4, orientation='V')

		x5, y5 = [ self.coords(x4,rfeed+wtot), self.coords(y4,l2+arc/2) ]
		resonator += self.rs.halfarc_trench(rfeed, w, g, x5, y5, orient='N')

		x6, y6 = [ self.coords(x5,rfeed), self.coords(y5) ]
		resonator += self.rs.straight_trench(-l3, w, g, x6, y6, orientation='V')

		x7, y7 = [ self.coords(x6,rfeed+wtot), self.coords(y6,-l3) ]
		resonator += self.rs.halfarc_trench(rfeed, w, g, x7, y7, orient='S')

		x8, y8 = [ self.coords(x7,rfeed), self.coords(y7) ]
		resonator += self.rs.straight_trench(l3, w, g, x8, y8, orientation='V')

		x9, y9 = [ self.coords(x8,rfeed+wtot), self.coords(y8,l3) ]
		resonator += self.rs.halfarc_trench(rfeed, w, g, x9, y9, orient='N')

		x10, y10 = [ self.coords(x9,rfeed), self.coords(y9) ]
		resonator += self.rs.straight_trench(-l3, w, g, x10, y10, orientation='V')

		x11, y11 = [ self.coords(x10,rfeed+wtot), self.coords(y10,-l3) ]
		resonator += self.rs.halfarc_trench(rfeed, w, g, x11, y11, orient='S')

		x12, y12 = [ self.coords(x11,rfeed), self.coords(y11) ]
		resonator += self.rs.straight_trench(l2, w, g, x12, y12, orientation='V')

		x13, y13 = [ self.coords(x12,rfeed+wtot), self.coords(y12,l2) ]
		resonator += self.rs.quarterarc_trench(rfeed, w, g, x13, y13, orient='NW')

		x14, y14 = [ self.coords(x13), self.coords(y13,rfeed) ]
		resonator += self.rs.straight_trench(l1, w, g, x14, y14, orientation='H')
